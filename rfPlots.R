...
generate_plot <- function(file,run,method,labels="Year"){
avg_fields <- read.csv(file)
...
if(label=="CFIDYr"){
  plot <- ggplot(avg_fields) + geom_point(aes(x=Actual, y = Predicted, 
                                              color=factor(CFIDYr), size=1),shape=1) + geom_abline(intercept=0, slope=1) +
    labs(x = "Actual yield (t/ha.)",y="Predicted yield (t/ha.)", title="Random Forest Actual Vs. Predicted Yield", color="CFIDYr") +
